...
def test_comment_cleaner():
    test_comments = [
        ...
        },
        {
            ...
            "username": "TestUser7",
            "comment": "This comment has exactly ten characters!",
            "video_id": "test_video_123",
            "channel_name": "Test Channel",
        },
    ]
    ...
    try:
        ...
        for i, comment in enumerate(test_comments, 1):
            ...
            print(
                f"✅ Sent test comment {i}/{len(test_comments)}: {comment['comment']}"
            )
            time.sleep(0.5)
        ...
        for message in consumer:
            ...
            print(
                f"✅ #{cleaned_count}: {comment_data['username']}: {comment_data['comment']}"
            )
        ...
        if len(test_comments) - cleaned_count == expected_filtered:
            ...
            print(
                f"   Expected {expected_filtered} comments to be filtered, got {len(test_comments) - cleaned_count}"
            )
        else:
            print(
