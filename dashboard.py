...
class Dashboard:
    ...
    def get_sentiment_counts(self):
        ...
        try:
            positive = int(self.redis_client.get(f"{config.SENTIMENT_CACHE_KEY}:counts:positive") or 0)
            negative = int(self.redis_client.get(f"{config.SENTIMENT_CACHE_KEY}:counts:negative") or 0)
            neutral = int(self.redis_client.get(f"{config.SENTIMENT_CACHE_KEY}:counts:neutral") or 0)
    ...
    def get_recent_comments(self, limit=20):
        ...
        try:
            ...
            for comment_id in comment_ids:
                comment_data = self.redis_client.get(f"{config.SENTIMENT_CACHE_KEY}:{comment_id}")
                if comment_data:
    ...
    def get_summary_history(self, limit=10):
        ...
        try:
            ...
            for summary_id in summary_ids:
                summary_data = self.redis_client.get(f"{config.SUMMARY_CACHE_KEY}:{summary_id}")
                if summary_data:
    ...
    def render_metrics(self):
        ...
        with col2:
            st.metric(
                ...
                delta=f"{(sentiment_counts['positive']/total_comments*100):.1f}%" if total_comments > 0 else "0%"
            )
        ...
        with col3:
            st.metric(
                ...
                delta=f"{(sentiment_counts['negative']/total_comments*100):.1f}%" if total_comments > 0 else "0%"
            )
        ...
        with col4:
            st.metric(
                ...
                delta=f"{(sentiment_counts['neutral']/total_comments*100):.1f}%" if total_comments > 0 else "0%"
            )
    ...
    def render_recent_comments(self):
        ...
        styled_df = df[['timestamp', 'username', 'comment', 'sentiment', 'confidence']].style.applymap(
    ...
    def render_latest_summary(self):
        ...
        with col2:
            window_start = datetime.fromisoformat(summary['window_start']).strftime('%H:%M')
            window_end = datetime.fromisoformat(summary['window_end']).strftime('%H:%M')
            st.metric("Window", f"{window_start} - {window_end}")
        
        # Summary text
        st.text_area("Summary", summary['summary'], height=150, disabled=True)
    ...
    def render_summary_history(self):
        ...
        for i, summary in enumerate(summaries):
            with st.expander(
                f"Summary {i+1} - {datetime.fromisoformat(summary['timestamp']).strftime('%H:%M:%S')} "
